...
void key_scanner(void)
{
    ...
    for(i = 0; i < KEY_NUMBER; i++)
    {
        ...
        if(KEY_RELEASE_LEVEL != current_level)  // 按键按下状态
        {
            ...
            {
                key_state[i] = KEY_LONG_PRESS; //按键计时长按
            }
            else//按按键被按下但是不满足长按状态时为短按状态
            {
                key_state[i] = KEY_PRESSED;  // 按键短按按下状态               
            }
        }
        else  // 按键释放状态
        {
...
{
//    last_key_state[key_n]= key_state[key_n]
//    key_state[key_n] = KEY_RELEASE;
//    return last_key_state[key_n];
