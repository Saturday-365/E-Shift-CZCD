...
## 提醒
1. **20260730**由于很多车队无法搭建或者不方便搭建能够读取档位信息的闭环系统，故在此提出一个假设的解决方案，即使用电机自带的力矩感知，通过换挡过程中的力矩感知实际档位有没有正常卡入，在卡入正常档位时电机转动角度可能不会突变，但此时的力矩即电流一定会有突变，通过软件识别这个突变来实现档位的感知，通过这个感知系统可以在单片机内构建当前档位数据，就也可以实现档位0-1-2-3...的序列式换挡
...
2. **20260605**  
  关于拉离合的电机，作这样几点说明 
    * 我们的离合电机在测试过程中，维持离合不联动状态时，电池压降高达2v，因此没有使用电离合进行起步
    * 为了追求极致的换挡速度，在换挡过程中靠转速匹配进行进档，没有使用离合的电机。因为离合电机接入的换挡逻辑涉及到等待离合到达指定位置在运作换挡电机（其实可以调的刚刚好让离合到位的同时马上进档）
    * 换挡带入离合电机可以提升换挡的成功率，但是也会产出这样一个问题，降档速度不匹配的话离合恢复联动，造成车辆后轮转速突变，可能产生车手无法预料的滑动（可以增加转速保护，在特定转速下禁止换挡）
    * 综上本队调试时间极其有限，希望后续能有更多的数据和测试
## 代码框架
