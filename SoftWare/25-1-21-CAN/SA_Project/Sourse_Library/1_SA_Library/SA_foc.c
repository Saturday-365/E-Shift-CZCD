...
void EN_Motor()
{
    HAL_GPIO_WritePin((GPIO_TypeDef *)EN1_GPIO_Port, (uint16_t)EN1_Pin, (GPIO_PinState)1); 
    HAL_GPIO_WritePin((GPIO_TypeDef *)EN2_GPIO_Port, (uint16_t)EN2_Pin, (GPIO_PinState)1); 
    HAL_GPIO_WritePin((GPIO_TypeDef *)EN3_GPIO_Port, (uint16_t)EN3_Pin, (GPIO_PinState)1); 
}
...
void M3_HAL_SETPWM(float a1,float b1,float c1)
{   
    __HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_4, a1*cvalue); //设置CH4->PWM脉冲宽度    
    __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1, b1*cvalue); //设置CH4->PWM脉冲宽度
    __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_2, c1*cvalue); //设置CH3->PWM脉冲宽度
}
void M2_HAL_SETPWM(float a2,float b2,float c2)
{   
    __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_3, a2*cvalue); //设置CH4->PWM脉冲宽度    
    __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_4, b2*cvalue); //设置CH4->PWM脉冲宽度
    __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_2, c2*cvalue); //设置CH3->PWM脉冲宽度
}
void M1_HAL_SETPWM(float a3,float b3,float c3)
{   
    __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, a3*cvalue); //设置CH4->PWM脉冲宽度   
    __HAL_TIM_SET_COMPARE(&htim5, TIM_CHANNEL_2, b3*cvalue); //设置CH4->PWM脉冲宽度
    __HAL_TIM_SET_COMPARE(&htim5, TIM_CHANNEL_1, c3*cvalue); //设置CH3->PWM脉冲宽度
}
...
void Motor_setPwm(float Ua,float Ub,float Uc,char ch) {
  ...
  if (ch==1){M1_HAL_SETPWM(dc_a,dc_b,dc_c);}
